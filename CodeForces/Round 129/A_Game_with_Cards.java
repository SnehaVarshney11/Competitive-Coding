...
public class A_Game_with_Cards{
    ...
    private static class Solution {
        public void solveTestcase(int testNumber, InputReader sc, OutputWriter out) {
            ...
            int n = sc.nextInt(); // the number of cards Alice has.
            int[] alice = new int[n]; // the numbers written on the cards that Alice has.
            for(int i = 0; i < n; i++){
                alice[i] = sc.nextInt();
            }
            
            int m = sc.nextInt(); // the number of Bob's cards.
            int[] bob = new int[m]; // the numbers on Bob's cards.
            for(int i = 0; i < m; i++){
                bob[i] = sc.nextInt();
            }
            int m1 = 0, m2 = 0;
            for(int i = 0; i < n; ++i){
                m1 = Math.max(m1, alice[i]);
            }
            for(int i = 0; i < m; ++i){
                m2 = Math.max(m2, bob[i]);
            }
            if(m1 == m2){
                out.println("Alice\nBob");
            }else if(m1 > m2){
