...
public class C_Unequal_Array{
    ...
    private static class Solution {
        public void solveTestcase(int testNumber, InputReader sc, OutputWriter out) {
            ...
            double count_E = 0, count_Z = 1e9;
            int i = 0;
            while(i < n-1){
                if(ar[i] == ar[i+1]){
                    count_Z = Math.min(count_Z, i);
                    count_E = Math.max(count_E, i);
                }
                ++i;
            }
            if(count_Z == count_E || count_Z == 1e9){
                out.println(0);
                return;
            }
            if(count_E - count_Z < 3){
                out.println(1);
            }else{
                out.println((int)(count_E-count_Z-1));
            }
