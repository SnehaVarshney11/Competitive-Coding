...
class Journey_of_the_Knight{
    ...
    private static class Solution {
            ...
            while(!ll.isEmpty()){
                ...
                while(s --> 0){
                    ...
                    for(int i = 0; i < direction.length; i++){
                        int row = rr + direction[i][0];
                        int col = rc + direction[i][1];
                        if(row >= 0 && col >= 0 && row < 8 && col < 8 && !visited[row][col]){
                            visited[row][col] = true;
                            if(row == er && col == ec){
                                return level;
                            }
                            ll.addLast(row*8 + col);
                        }
                    }
                }
                level++;
        ...
        public void solveTestcase(int testNumber, InputReader sc, OutputWriter out) {
            ...
            int er = sc.nextInt();
            int ec = sc.nextInt();
            int tmp = bfs(sr-1, scc-1, er-1, ec-1);
            if(tmp % 2 == 0){
                out.println("YES");
            }else{
                out.println("NO");
